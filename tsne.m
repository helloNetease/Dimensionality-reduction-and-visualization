...
% University of California, San Diego
    ...
    if numel(no_dims) > 1              %numel计算数组中元素个数
        initial_solution = true;
        ydata = no_dims;
        no_dims = size(ydata, 2);    %把ydata的列数赋给no_dims
    ...
    X = X - min(X(:));   %X（：）将X所有元素转换为一列
    X = X / max(X(:));    %归一化
    X = bsxfun(@minus, X, mean(X, 1)); %mean（X，1）求列均值，参数为2则是行均值，这一步是一列元素去均值化   
    ...
    if ~initial_solution
        ...
        [M, lambda] = eig(C);              %求C的特征值和特征向量，lambda和M
